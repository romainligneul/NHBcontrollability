...
% variations
clear all;
F.name = 'SPM12_R6RETROICOR_2ROI_HP96_prior1_SASpe'; % name of the GLM - first level
    ...
    for r = 1:4
        ...
        for tt = block_ind(r):block_ind(r+1)-1
            ...
            if out{s}.u(1,tt)==1 && tt~=block_ind(r)
                ...
                SASprob = muX{s}(in_obsf.hs.map.SAS{cur_c}(cur_s,:),tt);
                SSprob = muX{s}(in_obsf.hs.map.SS(cur_s,:),tt);                
                SASunc(t) = sum(-(SASprob.*log2(SASprob)));
                SSunc(t) = sum(-(SSprob.*log2(SSprob)));             
            else
                 SSpe(t) = NaN;
                 SASpe(t) = NaN;
                 SSunc(t)=NaN;
        ...
        std_SSunc = SSunc(cond_ind<2);
        std_SASunc = SASunc(cond_ind<2);
        prd_SASunc = SASunc(cond_ind==2);
        prd_SSunc = SSunc(cond_ind==2);
...
%         pmod(1).param{n} = zscore(std_SSunc); %zscore(std_SSpe'-std_SASpe');
        pmod(1).param{n} = zscore(std_SASpe');
        pmod(1).poly{n} = 1;
