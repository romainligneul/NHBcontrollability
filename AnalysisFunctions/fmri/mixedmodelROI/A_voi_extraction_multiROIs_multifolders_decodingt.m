...
basedir = '/project/3017049.01/SASSS_fMRI1/DECODING/DEC03_hp96_subj/';
subdirs = dir(basedir)
subdirs(1:2)=[];
% 
...
pattern_pref = 's3res_balanced_accuracy_minus_chance';
pattern_suff = '.nii';
...
for d = 1:length(subdirs)
    targetdir = [basedir subdirs(d).name '/'];
    filenames{d,1} = {[targetdir pattern_pref pattern_suff]};
end
...
roidir = '/project/3017049.01/SASSS_fMRI1/DECODING_LEVEL2/DEC03_hp96_subj/all_RTOmega_s3/s3res_balanced_accuracy_minus_chance/clusters_005/';
...
%            VOL_XYZmm(:,vx)=dum;
        ...
        [extdata] = spm_get_data(cellstr(filenames{d,1}{1}),XYZvx,true);
        
        % build the fields
        Roi = repmat(cellstr(roinames{r}(1:end-4)),size(extdata,1),1);
        Folder = repmat(cellstr(subdirs(d).name),size(extdata,1),1);        
        Target = filenames{d};
...
[r p] = corr(result_table.Mean,betas_X1(:,1))
