...
results_coordinates = '/project/3017049.01/SASSS_fMRI1/LEVEL2/SPM12_GLM30sasbis_R6RETROICOR_2ROI_HP96_bis/SS_SAS_intermodel/Table_PEconj_compactFWEpeaks.csv';
...
%     end
% end
% coords = [6 41 35];% coords(find(~ismember(1:size(distances,1),unique(remove_ROIS))),:)

% distances = squareform(pdist(coords, 'euclidean'));


labels = dumfile.textdata(2:end,1);
...
output_ROI_folder = '/project/3017049.01/SASSS_fMRI1/VOI_analysis/ROIimages/conjPEpeaks/';
mkdir(output_ROI_folder);


% add dependencies to path
addpath(genpath('/project/3017049.01/Tools/GPPI/'));
...
for v=1:size(coords,1)
    ...
    create_sphere_image(spmfile,coords(v,:),{[labels{v} '_' num2str(coords(v,1)) '_' num2str(coords(v,2)) '_' num2str(coords(v,3))]},diam);
    cd(pwd)
end
