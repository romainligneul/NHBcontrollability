function meanbycond = sembycond(data, cond, filter)
...
for c1 = 1:length(condtype{1})
    for c2 = 1:length(condtype{2})
        meanbycond(1,c) = std(data(cond(:,1) == condtype{1}(c1) & cond(:,2) == condtype{2}(c2)))/sqrt(sum(cond(:,1) == condtype{1}(c1) & cond(:,2) == condtype{2}(c2)));
        c = c +1;
    end;
end;
