...
%amean=smoothLNI(nanmean(amatrix),smth)';
...
if ischar(acolor)
    hold on;h=plot(F,amean,'color', acolor,'linewidth',1.5); %% change color or linewidth to adjust mean line
else
    hold on;h=plot(F,amean,'color', acolor-min(acolor),'linewidth',1.5); %% change color or linewidth to adjust mean line
end
