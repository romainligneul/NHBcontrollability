...
E.levels = [3]; % levels correspond to the target pattern length
E.r_bycond_bylevel = [5];
E.nchars = 10;
E.filler_lengths = {[0 1 2 3 4]+1, [0 1 2 3 4]+1}; 
...
E.trialdur = 1.5;
E.displaydur = 1;
E.duration_interblock_instructions = 3;
E.warningmsg_duration = 1.5;
...
for b = 1:E.nblocks
    ...
    if useful(t,1)==1 && L.keypressed(tt,1)~=0
        ...
        spst.text.str = [num2str(streak(t-2)) ' ' num2str(streak(t-1)) ' ' num2str(streak(t)) '\n\nGood, you got it in ' num2str(L.rt(tt,1),4) 's !'];
        eval(spst.text.exe);
        Screen('Flip', w.id);
...
save(['logfiles/WM/WMtraining_' S.fullid '.mat'], 'E', 'S', 'L');
try
save([external_folder 'WMtraining_' S.fullid '.mat'], 'S', 'L', 'E');
end
