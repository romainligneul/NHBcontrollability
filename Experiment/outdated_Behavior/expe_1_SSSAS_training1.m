...
%%% define instructions
E.instructions.dir =[E.stimdir 'instructions_training\'];
...
E.min_prediction_per_reversal = 6;
E.predict_maxstreak = 6; % on a 2-s explore / 4-s predict => max 200s per rev on a streak 10 basis and freqtest = 6
                          %(4 rev per run = 800s = 13 min)
                          % 4 x (13 + 1) = 56 min.
                          % localizer SS-SAS = 14 min
                          % + 7 min DTI
                          % + 6 min ANAT? get from database?
                          % + config = 10 min
                          % = 1h33 in scanner max. if learning in streak 7:
                          % 4*(6*4*2+6*7*2) = 132s => run = (9+1)*4 => 40
                          % = 1h15/1h25 should be the average
                          % / + 15 minutes training.
                          % / + 15 minutes preparation
                          % / + 10 minutes questionnaires
...
[E output] = instructions_tool(E,w, 'display', 1,2);
E.training2rule = [4 5 6 7;8 9 10 11]; 
% initialize run
...
for r = 5:8;
   ...
   % r = r +1;
    ...
    [E output] = instructions_tool(E,w, 'display', 12,12,12);
    ...
    save('emergency_training_log.mat', 'L', 'E');
    
end

instructions_tool(E,w, 'display', 3,3);
...
%         [pktdata, treceived(end+1)] = IOPort('Read', myport, 0, 1);
%     end
% end

filename = mfilename;
save(['training_outside_' S.fullid '.mat']);
Screen('CloseAll')
