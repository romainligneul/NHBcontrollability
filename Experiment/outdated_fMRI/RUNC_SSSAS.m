...
E.timing.fadedelay = 1.1;
E.timing.fadedur = 0.4;
...
E.noise = [0.05 0.10 0.20]; % [0 0.1 0.2] gives on average "at least 1 violation" in 50% of the sequences of 6
E.predictfrequency = 6;
E.min_prediction_per_reversal = 4;
E.predict_maxstreak = 10; % on a 2-s explore / 4-s predict => max 200s per rev on a streak 10 basis and freqtest = 6
...
E.predict.names = predictnames{S.tasktype(3)};
...
E.explore.names = explorenames{S.tasktype(3)};%{{'triangle_1', 'square_1'};{'triangle_2', 'circle_2'}; {'square_3', 'circle_3'}};
...
while current_time-L.start_time < E.timing.runtime && r<=4
    ...
    while L.criterion(r,1) == 0
        ...
        [explore_output choice] = exploratory_trial_good( E, w, state, side, E.post_stdjitter(t));
